...
- $\objt$ 为**终对象**当且仅当对任意 $\cat$ 中对象
   $\obj$ 都有且仅有唯一的箭头 $\getbang {\obj[x]}:\obj[x] \cathom \objt$ ; 
   ...
   \ar@[red]@{-->}[u]^[red]{\getid\obj=\getbang{\obj}\catcnst\cat{\obj[x]}\pila} 
...
- $\obji$ 为**始对象**当且仅当对任意 $\cat$ 中对象 
   ...
   \ar@[red]@{-->}[d]_[red]{\getabsurd{\obj[x]}~\catdiag\pila} 
   &
   {\obj[a]\mathrlap{\text{ 只能等于 }\obj[x]}\pila}
...
> $\catcnst\cat{\obj[x]}:\obj[x]\longmapsto \objt$ 。 另外还有一点 :
> 其他范畴中始终对象不一定存在 。
