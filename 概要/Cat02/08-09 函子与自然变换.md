...
- $\cat/\obj$ : **俯范畴** , 这里 $\obj$ 为任意 $\cat$ 中对象 ; 满足
  ...
  - $\obj[c_2]/\cat$ 中箭头皆形如 $\arr[f]\!\cancel{\smash[b]{\evlbin[]\cons
    ...
    $\obj[c]$ , $\obj[c']$ 为 $\cat$ 中任意对象且 $\arr[f]$ , $\arr[i]$ , $\arr[i']$ 为 $\cat$ 中任意箭头 ; ==TODO==
    $
...
若知 $\fct[F_2]:\evlbin[]{\cathom[\catCat]}{\evlcrynat[]\op\cat}\catSet$ 则
反变米田引理的陈述如下 :
...
2. $\Rightarrow$ : 考虑任意等式左侧的 $\ntf[\eta_1]$ : 若上述交换图成立
   则可对任意 $\ntf[\eta_1]$ 指派 $\bbox[LightGray]{\etc}=\bbox[LightGray]{\evlcry[]{\evlcrynat{\ntf[\eta_1]}
     {\obj}}
       {\id[{\obj[c_1]}]}}$ 为 $\evlcry[]{\bbox[LightGreen]{\fct[F_2]}}
         {\obj[c_2]}$ 中与之对应的元素 ;
...
若还知 $\fct[F_1]:\evlbin[]{\cathom[\catCat]}{\cat}\catSet$ 则
协变米田引理的陈述如下 :
...
- $\yoneda$ 是完全忠实的 , 因为
  将反变米田引理中的 $\fct[F_2]$ 
  换成 $\prs{\bbox[YellowGreen]{\evlbin[]{\cathom[{\evlcrynat[]\op\cat}]}
            {\obj[c_2']}
            {{}}}\wld}$ 
  即可获得下述公式 :
